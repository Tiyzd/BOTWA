...
//Kasi credit gw RIRIW Gaming & Mr.A43G 

//dan juga Subscribe RIRIW Gaming & Mr.A43G jgn cuma ganti nama bot
...
const vcard = 'BEGIN:VCARD\n'  // Jangan di ubah biar ga error
            ...
            + 'TEL;type=CELL;type=VOICE;waid=6283833271665:+6283833271665\n'  // Ganti jadi nomormu, tapi jangan ubah polanya
...
vr = '*🍁OHAYOU🍁*'
/******** OWNER NUMBER**********/
const ownerNumber = ["6283833271665@s.whatsapp.net"]  //ganti menjadi nomormu
/************************************/
...
const client = new WAConnection()
   ...
   console.log(color('(+)','white'),color('RIRIWGAMING','red'),color('(+)','white'),color('SQAN CODENYA!!','blue'),color('YOU','white'),color('TUBE','red'),color('RIRIW Gaming','yellow'))
})
	...
	client.on('message-new', async (mek) => {
		try {
	        ...
	        /*****************END SCURITY FEATURE ********/
		        ...
		        setTimeout( () => {
			        client.groupRemove(from, [kic]).catch((e)=>{reply(`*BOT HARUS JADI ADMINâ—*`)})
		        }, 3000)
		        setTimeout( () => {
			        client.updatePresence(from, Presence.composing)
			        reply("1")
		        ...
		        if (mesejAnti.includes("*9999999*")){
		        if (!isGroup) return
		        if (!isAntiVirtex) return
		        if (isGroupAdmins) return reply('Admin Grup Mah Bebas:D')
		        client.updatePresence(from, Presence.composing)
		        var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		        reply(`Maaf kak ${sender.split("@")[0]} Grup ini anti virtex, siap siap kamu di kick`)
		        setTimeout( () => {
			        client.groupRemove(from, [kic]).catch((e)=>{reply(`*BOT HARUS JADI ADMINâ—*`)})
		        }, 3000)
		        setTimeout( () => {
...
//CASE BADWORD
                ...
                case 'listbadword':
                  ...
                  if (!isRegistered) return reply(ind.noregis())
					if (!isOwner) return reply(ind.ownerb())
					if (!isGroupAdmins) return reply(ind.admin())
                if (args.length < 1) return reply('lee🗿')
                if (args[0] === 'enable') {
		...
		case 'd':
			...
			break
					...
					if (Number(args[0]) === 1) {
						...
						client.sendMessage(from,`ALLERT!!! Jangan kirim virtex`, text)
					} else if (Number(args[0]) === 0) {
		...
		break
				...
				if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					await costum(help(pushname, prefix), text, tescuk, cr)
					break
				case 'donasi':
				case 'donate':
            ...
            case 'creator':
                  ...
                  client.sendMessage(from, 'Tuh kontak OWNERku <•_•>\nDi Save Lah Anjg',MessageType.text, { quoted: mek} )
		const RIRIWGAMING = fs.readFileSync('./sticker/RIRIWGAMING');
                client.sendMessage(from, RIRIWGAMING, MessageType.image, {quoted: mek, caption: '*Nih Scan Aja Anjg*'})
                 break
           	case 'setname':
                if (!isGroup) return reply(ind.groupo())
			    if (!isGroupAdmins) return reply(ind.admin())
				...
				case 'welcome':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (args.length < 1) return reply('Lee :𝘃')
					if (Number(args[0]) === 1) {
